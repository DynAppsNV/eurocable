...
class Base(models.AbstractModel):
    ...
    def _fetch_query(self, query, fields):
        records = super(Base, self)._fetch_query(query, fields)
        ...
        if self._context.get("history_revision"):
            ...
            if audit_rules:
                history_date = fields.Datetime.from_string(self._context.get("history_revision"))
                ...
                for record in records:
                    ...
                    for log in logs:
                        if log.res_id == record.id:
                            data = safe_eval(log.data or "{}", {"datetime": datetime})
