...
class ColorFormatter(logging.Formatter):
    def __init__(self, form="%(message)s"):
        """Create a new formatter that adds colors"""
        super().__init__(form)

    def format(self, record, *args, **kwargs):
        if record.levelno in LOG_COLORS:
            record.msg = LOG_COLORS[record.levelno] + record.msg + colorama.Style.RESET_ALL
...
def reset_passwords(c, database_name="", password="admin"):
    ...
    query = query.replace("'", "'\\''")
    _pg_run(c, f"psql {database_name} -c '{query}'")
...
def lint_flake8(c, addons=""):
    ...
    flakerc = ".flake8"
    for path in paths:
        command = f"flake8 {path} --config={flakerc}"
        c.run(command)
...
def lint_odoo_lint(c, addons=""):
    ...
    command = (
        "pylint --errors-only --load-plugins=pylint_odoo -d all -e odoolint" + " %s --disable=%s "
    )
...
def lint_xml(c, addons=""):
    ...
    if os.name == "nt":
        ...
        return
    c.run('find . -maxdepth 4 -type f -iname "*.xml" ' "| xargs -I '{}' xmllint -noout '{}'")
...
def _get_addons_from_directory(directory):
    return [
        f[len(directory) + 1 :].split("/")[0] for f in glob.glob(directory + "/*/__manifest__.py")
    ]
...
def github_clean(c):
    ...
    for remote in ["origin", "github"]:
        c.run(f"git fetch --prune {remote}")
    ...
    for branch in branches_to_remove:
        c.run(f"git push github --delete '{branch}'")
